use anyhow::anyhow;
use fusion_auction::auction::dutch::actions::{
    ActionDutchAuctionEnd, ActionDutchAuctionSchedule, ActionDutchAuctionWithdraw,
};
use fusion_txhash::{EffectHash, EffectingData};
use std::convert::{TryFrom, TryInto};

use fusion_asset::balance;
use fusion_proto::{core::transaction::v1 as pb, DomainType};

use crate::{ActionView, IsAction, TransactionPerspective};
use serde::{Deserialize, Serialize};

/// An action performed by a Fusion transaction.
#[derive(Clone, Debug, Deserialize, Serialize)]
#[serde(try_from = "pb::Action", into = "pb::Action")]
#[allow(clippy::large_enum_variant)]
pub enum Action {
    Output(fusion_shielded_pool::Output),
    Spend(fusion_shielded_pool::Spend),
    ValidatorDefinition(fusion_stake::validator::Definition),
    IbcRelay(fusion_ibc::IbcRelay),
    Swap(fusion_dex::swap::Swap),
    SwapClaim(fusion_dex::swap_claim::SwapClaim),
    ProposalSubmit(fusion_governance::ProposalSubmit),
    ProposalWithdraw(fusion_governance::ProposalWithdraw),
    DelegatorVote(fusion_governance::DelegatorVote),
    ValidatorVote(fusion_governance::ValidatorVote),
    ProposalDepositClaim(fusion_governance::ProposalDepositClaim),

    PositionOpen(fusion_dex::lp::action::PositionOpen),
    PositionClose(fusion_dex::lp::action::PositionClose),
    PositionWithdraw(fusion_dex::lp::action::PositionWithdraw),

    Delegate(fusion_stake::Delegate),
    Undelegate(fusion_stake::Undelegate),
    UndelegateClaim(fusion_stake::UndelegateClaim),

    Ics20Withdrawal(fusion_shielded_pool::Ics20Withdrawal),

    CommunityPoolSpend(fusion_community_pool::CommunityPoolSpend),
    CommunityPoolOutput(fusion_community_pool::CommunityPoolOutput),
    CommunityPoolDeposit(fusion_community_pool::CommunityPoolDeposit),
    ...
    ActionLiquidityTournamentVote(
        fusion_funding::liquidity_tournament::ActionLiquidityTournamentVote,
    ),
}
