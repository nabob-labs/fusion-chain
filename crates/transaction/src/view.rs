...
use fusion_asset::{Balance, Value};
use fusion_dex::{swap::SwapView, swap_claim::SwapClaimView};
use fusion_keys::AddressView;
use fusion_proto::{core::transaction::v1 as pbt, DomainType};
use fusion_shielded_pool::{OutputView, SpendView};
...
mod test {
    ...
    use fusion_dex::swap::proof::SwapProof;
    use fusion_dex::swap::{SwapCiphertext, SwapPayload};
    use fusion_dex::Swap;
    use fusion_dex::{
    ...
    use fusion_fee::Fee;
    use fusion_keys::keys::Bip44Path;
    use fusion_keys::keys::{SeedPhrase, SpendKey};
    use fusion_keys::{
        symmetric::{OvkWrappedKey, WrappedMemoKey},
        test_keys, Address, FullViewingKey, PayloadKey,
    };
    use fusion_num::Amount;
    use fusion_proof_params::GROTH16_PROOF_LENGTH_BYTES;
    use fusion_sct::Nullifier;
    use fusion_shielded_pool::Rseed;
    use fusion_shielded_pool::{output, spend, Note, NoteView, OutputPlan, SpendPlan};
    use fusion_tct::structure::Hash;
    use fusion_tct::StateCommitment;
    use rand_core::OsRng;
    use std::ops::Deref;
    ...
    fn dummy_proof_spend() -> spend::SpendProof {
        spend::SpendProof::try_from(
            fusion_proto::fusion::core::component::shielded_pool::v1::ZkSpendProof {
    ...
    fn dummy_proof_output() -> output::OutputProof {
        output::OutputProof::try_from(
            fusion_proto::fusion::core::component::shielded_pool::v1::ZkOutputProof {
                inner: vec![0u8; GROTH16_PROOF_LENGTH_BYTES],
            },
        )
        .expect("creating a dummy proof should work")
    ...
    fn dummy_proof_swap() -> SwapProof {
        SwapProof::try_from(
            fusion_proto::fusion::core::component::dex::v1::ZkSwapProof {
                inner: vec![0u8; GROTH16_PROOF_LENGTH_BYTES],
            },
        )
        .expect("creating a dummy proof should work")
    ...
    fn dummy_spend() -> spend::Spend {
        use fusion_shielded_pool::EncryptedBackref;
    ...
    fn dummy_output() -> output::Output {
        output::Output {
            body: output::Body {
                note_payload: fusion_shielded_pool::NotePayload {
                    note_commitment: fusion_shielded_pool::note::StateCommitment(
                        Fq::default(),
                    ),
                    ...
                    encrypted_note: fusion_shielded_pool::NoteCiphertext([0u8; 176]),
                },
                balance_commitment: dummy_commitment(),
    ...
    fn convert_action(
        ...
        match action {
            ActionPlan::Output(x) => Some(ActionView::Output(
                fusion_shielded_pool::OutputView::Visible {
                    output: dummy_output(),
    ...
    fn test_internal_transfer_transaction_summary() {
        ...
        let transaction_view = TransactionView {
            anchor: fusion_tct::Root(Hash::zero()),
