...
use fusion_keys::{Address, FullViewingKey, PayloadKey};
...
impl TransactionPlan {
    ...
    pub fn validator_definitions(
        &self,
    ) -> impl Iterator<Item = &fusion_stake::validator::Definition> {
        self.actions.iter().filter_map(|action| {
...
#[cfg(test)]
mod tests {
    use fusion_asset::{asset, Value, STAKING_TOKEN_ASSET_ID};
    use fusion_dex::{swap::SwapPlaintext, swap::SwapPlan, TradingPair};
    use fusion_fee::Fee;
    use fusion_keys::{
        keys::{Bip44Path, SeedPhrase, SpendKey},
        Address,
    };
    use fusion_shielded_pool::Note;
    use fusion_shielded_pool::{OutputPlan, SpendPlan};
    use fusion_tct as tct;
    use fusion_txhash::EffectingData as _;
    use rand_core::OsRng;
    ...
    fn plan_effect_hash_matches_transaction_effect_hash() {
        ...
        let mut plan: TransactionPlan = TransactionPlan {
            ...
            transaction_parameters: TransactionParameters {
                expiry_height: 0,
                fee: Fee::default(),
                chain_id: "fusion-test".to_string(),
            },
