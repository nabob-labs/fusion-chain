...
use fusion_asset::{asset, EstimatedPrice, Value, ValueView};
use fusion_dex::BatchSwapOutputData;
use fusion_keys::{Address, AddressView, PayloadKey};
use fusion_proto::core::transaction::v1::{
    self as pb, NullifierWithNote, PayloadKeyWithCommitment,
};
use fusion_sct::Nullifier;
use fusion_shielded_pool::{note, Note, NoteView};
        ...
        for denom in msg.denoms {
            denoms.insert(
                denom
                    .fusion_asset_id
                    .clone()
                    .ok_or_else(|| anyhow!("missing fusion asset ID in denom"))?
                    .try_into()?,
                denom.try_into()?,
            );
        }
