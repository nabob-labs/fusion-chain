...
use fusion_app::app_version::check_and_update_app_version;
use fusion_app::{APP_VERSION, SUBSTORE_PREFIXES};
use fusion_tower_trace::remote_addr;
use rand::Rng;
use rand_core::OsRng;
...
async fn main() -> anyhow::Result<()> {
    ...
    match cmd {
        RootCommand::Start {
            ...
            let fnsd_home = match home {
                Some(h) => h,
                None => get_network_dir(None).join("node0").join("fnsd"),
            };
            let rocksdb_home = fnsd_home.join("rocksdb");

            let storage = Storage::load(rocksdb_home, SUBSTORE_PREFIXES.to_vec())
                .await
                .context(
                    "Unable to initialize RocksDB storage - is there another `fnsd` process running?",
                )?;
            ...
            if fusion_app::app::App::is_ready(storage.latest_snapshot()).await {
            ...
            let abci_server = tokio::task::spawn(
                fusion_app::server::new(storage.clone()).listen_tcp(abci_bind),
            );

            let tm_proxy = fusion_tendermint_proxy::TendermintProxy::new(cometbft_addr);

            let grpc_routes =
                fusion_app::rpc::routes(&storage, tm_proxy, enable_expensive_rpc)?
                    .into_axum_router()
                    .layer(
            ...
            let frontend = fnsd::zipserve::router("/app/", fnsd::MINIFRONT_ARCHIVE_BYTES);
            let node_status = fnsd::zipserve::router("/", fnsd::NODE_STATUS_ARCHIVE_BYTES);

            // Now we drop down a layer of abstraction, from tonic to axum, and merge handlers.
            let router = grpc_routes
                .merge(frontend)
                .merge(node_status)
                // Set rather permissive CORS headers for fnsd's gRPC: the service
            ...
            let (grpc_server, acme_worker) = match grpc_auto_https {
                Some(domain) => {
                    let (acceptor, acme_worker) =
                        fusion_auto_https::axum_acceptor(fnsd_home, domain, !acme_staging);
            ...
            use fusion_dex::component::metrics::PrometheusBuilderExt;
            ...
            // Spawn the HTTP service that lets Prometheus pull metrics from `fnsd`, and then
            // register fnsd's metrics with the exporter.
            tokio::spawn(exporter);
            fnsd::register_metrics();
            tokio::spawn(fnsd::metrics::sleep_worker::run());
            tokio::spawn(fnsd::metrics::cpu_worker::run());
        ...
        RootCommand::Network {
            ...
            if let Some(archive_url) = archive_url {
                fnsd::network::join::unpack_state_archive(archive_url, output_dir, leave_archive)
                    .await?;
            }
        }
        ...
        RootCommand::Network {
            ...
            let chain_id = match preserve_chain_id {
                true => chain_id.unwrap_or_else(|| env!("FNSD_LATEST_TESTNET_NAME").to_string()),
                false => {
                    // If preserve_chain_id is false, we append a random suffix to avoid collisions
                    let randomizer = OsRng.gen::<u32>();
                    let chain_id =
                        chain_id.unwrap_or_else(|| env!("FNSD_LATEST_TESTNET_NAME").to_string());
        ...
        RootCommand::Export {
            ...
            if let Some(archive_filepath) = export_archive {
                fnsd::migrate::archive_directory(
        ...
        RootCommand::Migrate {
            ...
            let (fnsd_home, comet_home) = match home {
                Some(h) => (h, comet_home),
                None => {
                    // If no fnsd_home was configured, we're assuming we set up the
                    // data in the default location, in which case we also know where comet lives.
                    let base = get_network_dir(None).join("node0");
                    (base.join("fnsd"), Some(base.join("cometbft")))
                }
            };
            let fnsd_migrate_span = tracing::error_span!("fnsd_migrate");
            fnsd_migrate_span
                .in_scope(|| tracing::info!("migrating fnsd state in {}", fnsd_home.display()));

            if ready_to_start {
                tracing::info!("disabling halt order in local state");
                ReadyToStart
                    .migrate(fnsd_home, comet_home, None, force)
