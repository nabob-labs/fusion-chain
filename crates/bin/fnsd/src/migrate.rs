...
//!
//! When consensus-breaking changes are made to the Fusion software,
//! node operators must coordinate to perform a chain upgrade.
//! This module declares how local `fnsd` state should be altered, if at all,
...
use fusion_governance::StateReadExt;
use fusion_sct::component::clock::EpochRead;
use std::path::{Path, PathBuf};
use tracing::instrument;

use cnidarium::Storage;
use fusion_app::SUBSTORE_PREFIXES;
...
impl Migration {
    #[instrument(skip(fnsd_home, genesis_start, force))]
    pub async fn migrate(
        ...
        let rocksdb_dir = fnsd_home.join("rocksdb");
        ...
        if let Migration::ReadyToStart = self {
            reset_halt_bit::migrate(storage, fnsd_home, genesis_start).await?;
            return Ok(());
        }

        match self {
            Migration::SimpleMigration => {
                simple::migrate(storage, fnsd_home.clone(), genesis_start).await?
            }
            Migration::Mainnet1 => {
                mainnet1::migrate(storage, fnsd_home.clone(), genesis_start).await?;
            }
            Migration::Mainnet2 => {
