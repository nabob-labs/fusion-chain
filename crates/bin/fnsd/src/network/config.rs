...
use fusion_app::genesis::AppState;
use fusion_custody::soft_kms::Config as SoftKmsConfig;
use fusion_keys::keys::{SpendKey, SpendKeyBytes};
use rand::Rng;
use rand_core::OsRng;
...
impl NetworkTendermintConfig {
    ...
    pub fn write_config(
        ...
        // We'll also create the fnsd state directory here, since it's convenient.
        let fnsd_dir = node_dir.clone().join("fnsd");
...
/// Collection of all keypairs required for a Fusion validator.
/// Used to generate a stable identity for a [`NetworkValidator`].
#[derive(Deserialize)]
pub struct ValidatorKeys {
    /// Fusion spending key and viewing key for this node.
...
pub fn get_network_dir(network_dir: Option<PathBuf>) -> PathBuf {
    // By default output directory will be in `~/.fusion/network_data/`
    match network_dir {
        Some(o) => o,
        None => canonicalize_path("~/.fusion/network_data"),
    }
}
