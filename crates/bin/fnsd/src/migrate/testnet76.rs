...
use fusion_governance::StateWriteExt;
use fusion_proto::{DomainType, StateReadProto, StateWriteProto};
use fusion_sct::component::clock::{EpochManager, EpochRead};
use fusion_shielded_pool::params::ShieldedPoolParameters;
use fusion_shielded_pool::{component::StateWriteExt as _, fmd::Parameters as FmdParameters};
...
async fn heal_auction_vcb(delta: &mut StateDelta<Snapshot>) -> anyhow::Result<()> {
    let key_prefix_auctions = fusion_auction::state_key::auction_store::prefix();
    ...
    for value in total_vcb.provided() {
        ...
        let key_vcb_balance =
            fusion_auction::state_key::value_balance::for_asset(&value.asset_id);
