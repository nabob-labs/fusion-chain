...
use jmt::RootHash;
use fusion_app::app::StateReadExt as _;
use fusion_dex::component::{PositionManager, StateReadExt, StateWriteExt};
use fusion_dex::lp::position;
use fusion_dex::lp::position::Position;
use fusion_governance::proposal_state::State as ProposalState;
use fusion_governance::Proposal;
use fusion_governance::StateReadExt as _;
use fusion_governance::StateWriteExt as _;
use fusion_proto::core::component::governance::v1 as pb_governance;
use fusion_proto::{StateReadProto, StateWriteProto};
use fusion_sct::component::clock::EpochManager;
use fusion_sct::component::clock::EpochRead;
use fusion_stake::validator::Validator;
...
pub async fn migrate(
    ...
    let genesis_path = fnsd_home.join("genesis.json");
    std::fs::write(genesis_path, genesis_json).expect("can write genesis");

    tracing::info!("updating signing state");
    let validator_state_path = fnsd_home.join("priv_validator_state.json");
...
async fn reindex_dex_positions(delta: &mut StateDelta<Snapshot>) -> anyhow::Result<()> {
    ...
    let prefix_key_lp = fusion_dex::state_key::all_positions();
    ...
    while let Some(lp) = stream_open_lp.next().await {
        ...
        delta.delete(fusion_dex::state_key::position_by_id(&id));
...
async fn truncate_validator_fields(delta: &mut StateDelta<Snapshot>) -> anyhow::Result<()> {
    ...
    let key_prefix_validators = fusion_stake::state_key::validators::definitions::prefix();
    let all_validators = delta
        .prefix_proto::<fusion_proto::core::component::stake::v1::Validator>(
            &key_prefix_validators,
        )
        .try_collect::<Vec<(
            String,
            fusion_proto::core::component::stake::v1::Validator,
        )>>()
        .await?;
    ...
    for proposal_id in 0..next_proposal_id {
        ...
        let proposal = delta
            .get_proto::<pb_governance::Proposal>(
                &fusion_governance::state_key::proposal_definition(proposal_id),
            )
            .await?;
    ...
    for proposal_id in 0..next_proposal_id {
        ...
        let proposal_state = delta
            .get_proto::<pb_governance::ProposalState>(
                &fusion_governance::state_key::proposal_state(proposal_id),
            )
            .await?;
