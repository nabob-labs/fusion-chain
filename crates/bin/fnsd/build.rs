...
fn setup_testnet_config() -> anyhow::Result<()> {
    ...
    println!("cargo:rustc-env=FNSD_LATEST_TESTNET_NAME={latest_testnet_name}");

    // Ensure that changes to the allocations files trigger a rebuild of fnsd.
    println!(
    ...
    for (env_var, filename) in [
        ("FNSD_LATEST_TESTNET_ALLOCATIONS", "allocations.csv"),
        ("FNSD_LATEST_TESTNET_VALIDATORS", "validators.json"),
    ] {
...
fn latest_testnet(testnets_path: impl AsRef<Path>) -> anyhow::Result<(String, String)> {
    ...
    testnets
        ...
        .map(|(_, name, dir_name)| ("fusion-testnet-".to_string() + &name, dir_name))
        .ok_or_else(|| {
            anyhow::anyhow!(
