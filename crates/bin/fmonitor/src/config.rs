//! Logic for reading and writing config files for `fmonitor`, in the TOML format.
use anyhow::Result;
use regex::Regex;
...
#[derive(Clone, Debug, Serialize, Deserialize)]
/// Representation of a single Fusion wallet to track.
pub struct AccountConfig {
    /// The initial [FullViewingKey] has specified during `fmonitor init`.
    ///
    ...
    /// List of account migrations, performed via `fcli migrate balance`, if any.
...
#[derive(Clone, Debug, Serialize, Deserialize)]
/// The primary TOML file for configuring `fmonitor`, containing all its account info.
///
/// During `fmonitor audit` runs, the config will be automatically updated
