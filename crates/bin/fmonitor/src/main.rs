...
//!     fmonitor audit
//!
//! If regular auditing is desired, consider automating the `fmonitor audit` action via
//! cron or similar. `fmonitor` will cache view databases for each tracked FVK, so that future
...
use fusion_asset::STAKING_TOKEN_ASSET_ID;
...
use fcli::config::FcliConfig;
use fusion_compact_block::CompactBlock;
use fusion_keys::FullViewingKey;
use fusion_num::Amount;
use fusion_proto::box_grpc_svc;
use fusion_proto::view::v1::{
...
use fusion_proto::{
    ...
    fusion::core::component::compact_block::v1::query_service_client::QueryServiceClient as CompactBlockQueryServiceClient,
};
use fusion_stake::rate::RateData;
use fusion_stake::DelegationToken;
use fusion_view::{Storage, ViewClient, ViewServer};

mod config;
mod genesis;

use config::{parse_dest_fvk_from_memo, AccountConfig, FvkEntry, FmonitorConfig};
...
const VIEW_FILE_NAME: &str = "fcli-view.sqlite";
...
fn init_tracing() -> anyhow::Result<()> {
    ...
    let filter_layer = EnvFilter::try_from_default_env()
        .or_else(|_| EnvFilter::try_new("info,fusion_view=off"))?;
...
/// The path to the default `fmonitor` home directory.
///
/// Can be overridden on the command-line via `--home`.
pub fn default_home() -> Utf8PathBuf {
    let path = ProjectDirs::from("nabob", "fusion", "fmonitor")
...
#[clap(
    name = "fmonitor",
    about = "The Fusion account activity monitor.",
    version
)]
pub struct Opt {
    /// Command to run.
    #[clap(subcommand)]
    pub cmd: Command,
    /// The path used to store fmonitor state.
    #[clap(long, default_value_t = default_home(), env = "FUSION_FMONITOR_HOME")]
    pub home: Utf8PathBuf,
}

#[derive(Debug, clap::Subcommand)]
pub enum Command {
    /// Generate configs for `fmonitor`.
    Init {
    ...
    Audit {},
    /// Delete `fmonitor` storage to reset local state.
    Reset {},
}
...
impl Opt {
    ...
    pub async fn create_wallet(
        ...
        // Use FVK to build a fcli config file,
        // which we'll reference when syncing wallets.
        let fcli_config = FcliConfig {
            grpc_url: grpc_url.clone(),
            view_url: None,
            governance_custody: None,
            full_viewing_key: fvk.clone(),
            disable_warning: true,
            custody: fcli::config::CustodyConfig::ViewOnly,
        };

        let fcli_config_path = wallet_dir.join("config.toml");
        fcli_config.save(fcli_config_path).with_context(|| {
    ...
    pub async fn exec(&self) -> Result<()> {
        ...
        match &opt.cmd {
            Command::Reset {} => {
                ...
                println!(
                    "Successfully cleaned up fmonitor directory: \"{}\"",
                    opt.home
                );
                Ok(())
            }
            Command::Init { fvks, grpc_url } => {
                ...
                if !opt.home.exists() {
                    ...
                    anyhow::bail!("fmonitor home directory already exists: {}", opt.home);
                }
                ...
                let fmonitor_config = FmonitorConfig::new(grpc_url.clone(), accounts);

                // Save the config
                let config_path = opt.home.join("fmonitor_config.toml");
                fs::write(config_path, toml::to_string(&fmonitor_config)?)?;
            ...
            Command::Audit {} => {
                ...
                for (index, config) in fmonitor_config.accounts().iter().enumerate() {
                    ...
                    if migration_tx.is_empty() {
                        ...
                        let wallet_id = Uuid::new_v4();
                        let wallet_dir = self.wallet_path(&wallet_id);
                        self.create_wallet(&wallet_dir, &new_fvk, &fmonitor_config.grpc_url())
                            .await?;
                        ...
                        view_client = self
                            .view(wallet_dir, new_fvk.clone(), fmonitor_config.grpc_url())
                            .await?;
                ...
                emit_summary_message(fmonitor_config.accounts(), failures)?;
...
/// if `fcli migrate balance` was used.
