...
use fusion_governance::ValidatorVoteBody;
use fusion_proto::{
...
use fusion_transaction::{txhash::TransactionId, Transaction, TransactionPlan};
use fusion_view::{ViewClient, ViewServer};
...
impl App {
    ...
    pub fn build_transaction(
        ...
        let tx = fusion_wallet::build_transaction(
    ...
    /// `fnsd` endpoint, as configured for `fcli`.
    pub async fn fnsd_channel(&self) -> anyhow::Result<Channel> {
        ViewServer::get_fnsd_channel(self.config.grpc_url.clone())
            .await
    ...
    pub async fn tendermint_proxy_client(
        ...
        let channel = self.fnsd_channel().await?;
