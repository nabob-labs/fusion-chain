    ...
    pub async fn exec(&self, app: &mut App) -> Result<()> {
        match self {
            IbcCmd::Client { client_id } => {
                let mut ibc_client = ClientQueryClient::new(app.fnsd_channel().await?);
            ...
            IbcCmd::Connection { connection_id } => {
                let mut ibc_client = ConnectionQueryClient::new(app.fnsd_channel().await?);
            ...
            IbcCmd::Channel { port, channel_id } => {
                let mut channel_client = ChannelQueryClient::new(app.fnsd_channel().await?);
                let mut connection_client = ConnectionQueryClient::new(app.fnsd_channel().await?);
                let mut client_client = ClientQueryClient::new(app.fnsd_channel().await?);
