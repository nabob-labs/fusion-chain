...
use fusion_proto::core::component::{
    dex::v1::{
...
pub enum DexCmd {
    ...
    BatchOutputs {
        ...
        /// Pairs must be specified with a colon separating them, e.g. "fusion:test_usd".
...
impl DexCmd {
    ...
    pub async fn get_simulated_execution(
        ...
        use fusion_proto::core::component::dex::v1::simulate_trade_request::{
            routing::Setting, Routing,
        };
        let mut client = SimulationServiceClient::new(app.fnsd_channel().await?);
        client
    ...
    pub async fn print_batch_outputs(
        ...
        let mut client = ShieldedPoolQueryServiceClient::new(app.fnsd_channel().await?);
    ...
    pub async fn exec(&self, app: &mut App) -> Result<()> {
        match self {
            ...
            DexCmd::AllPositions { include_closed } => {
                let client = DexQueryServiceClient::new(app.fnsd_channel().await?);
