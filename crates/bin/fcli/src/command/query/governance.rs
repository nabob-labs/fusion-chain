...
use fusion_governance::Vote;
use fusion_proto::core::component::governance::v1::{
            ...
            GovernanceCmd::Proposal { proposal_id, query } => {
                match query {
                    ...
                    PerProposalCmd::Tally => {
                        ...
                        > = client
                            ...
                            .map_ok(|response| {
                                let identity_key: IdentityKey = response
                                    ...
                                    .expect("identity key must be set for vote response")
                                    .try_into()?;
                                let tally: fusion_governance::Tally = response
                                    .tally
                                    .expect("tally must be set for vote response")
                                    .try_into()?;
                                Ok::<(IdentityKey, fusion_governance::Tally), anyhow::Error>(
                                    (identity_key, tally),
                                )
                            })
                        ...
                        let mut total = fusion_governance::Tally::default();
                        ...
                        for (identity_key, (vote, power)) in validator_votes_and_power.into_iter() {
                            all_votes_and_power.insert(identity_key.to_string(), {
                                ...
                                let sub_total = fusion_governance::Tally::from((
                                    vote,
...
fn json_tally(tally: &fusion_governance::Tally) -> serde_json::Value {
    let mut map = serde_json::Map::new();
    if tally.yes() > 0 {
