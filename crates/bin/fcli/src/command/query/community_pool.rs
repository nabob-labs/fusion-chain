...
use fusion_proto::{
    ...
    fusion::core::component::community_pool::v1::query_service_client::QueryServiceClient as CommunityPoolQueryServiceClient,
};
    ...
    pub async fn print_balance(&self, app: &mut App, asset: &Option<String>) -> Result<()> {
        let asset_id = asset.as_ref().map(|asset| {
            ...
            if let Ok(asset_id) = asset.parse() {
                ...
                fusion_asset::asset::REGISTRY
                    .parse_unit(asset.as_str())
                    .id()
            }
        });

        let mut client = CommunityPoolQueryServiceClient::new(app.fnsd_channel().await?);
        let balances = client
