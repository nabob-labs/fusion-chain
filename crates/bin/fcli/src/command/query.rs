...
impl QueryCmd {
    pub async fn exec(&self, app: &mut App) -> Result<()> {
        ...
        if let QueryCmd::ShieldedPool(ShieldedPool::CompactBlock { height }) = self {
            use fusion_proto::core::component::compact_block::v1::{
                query_service_client::QueryServiceClient as CompactBlockQueryServiceClient,
                CompactBlockRequest,
            };
            let mut client = CompactBlockQueryServiceClient::new(app.fnsd_channel().await?);
        ...
        let mut client = QueryServiceClient::new(app.fnsd_channel().await?);
