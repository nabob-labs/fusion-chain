...
use fusion_custody::threshold;
use fusion_keys::keys::{Bip44Path, SeedPhrase, SpendKey};
use rand_core::OsRng;
use termion::screen::IntoAlternateScreen;
use url::Url;

use crate::{
    config::{CustodyConfig, GovernanceCustodyConfig, FcliConfig},
...
pub struct InitCmd {
    ...
    #[clap(
        ...
        // fcli init inside of the test harness (where we override that)
        // will correctly set the URL, even though we don't subsequently
        // read it from the environment.
        env = "FUSION_NODE_FNSD_URL",
...
pub enum InitTopSubCmd {
    #[clap(flatten)]
    Spend(InitSubCmd),
    /// Initialize `fcli` in view-only mode, without spending keys.
    #[clap(display_order = 200)]
    ViewOnly {
        /// The full viewing key for the wallet to view.
        full_viewing_key: String,
    },
    /// Initialize a separate validator governance key for an existing `fcli` configuration (this
    /// option is only meaningful for validators).
    #[clap(subcommand, display_order = 300)]
    ValidatorGovernanceSubkey(InitSubCmd),
    /// Wipe all `fcli` configuration and data, INCLUDING KEYS.
...
pub enum ThresholdInitCmd {
    ...
    ///
    /// Unlike the other `fcli init` commands, this one ignores the global
    ...
    Deal {
        ...
        /// for that signer's fcli instance.  This implicitly specifies the
...
fn exec_deal(
    ...
    for config_path in home.iter() {
        ...
        if let InitType::GovernanceKey = init_type {
            let config = FcliConfig::load(&config_path)?;
    ...
    for (i, (config, config_path)) in configs.into_iter().zip(home.iter()).enumerate() {
        ...
        let config = if let InitType::SpendKey = init_type {
            ...
            let mut fcli_config = FcliConfig::load(config_path.join(crate::CONFIG_FILE_NAME))?;
            if fcli_config.governance_custody.is_some() {
                anyhow::bail!(
                    "governance key already exists in config file at {:?}; refusing to overwrite it",
                    config_path
                );
            }
            fcli_config.governance_custody = Some(GovernanceCustodyConfig::Threshold(config));
            fcli_config
        };
    ...
    pub async fn exec(&self, home_dir: impl AsRef<camino::Utf8Path>) -> Result<()> {
        ...
        let existing_config = {
            ...
            if config_path.exists() {
                Some(FcliConfig::load(config_path)?)
            } else {
                None
            }
        };
        ...
        let (full_viewing_key, custody) = match (&init_type, &subcmd, relevant_config_exists) {
            (_, InitSubCmd::SoftKms(cmd), false) => {
                ...
                (
                    ...
                    if self.encrypted {
                        ...
                        CustodyConfig::Encrypted(fusion_custody::encrypted::Config::create(
                            &password,
                            fusion_custody::encrypted::InnerConfig::SoftKms(spend_key.into()),
                        )?)
                    } else {
            ...
            (
                ...
                let custody_config = if self.encrypted {
                    ...
                    CustodyConfig::Encrypted(fusion_custody::encrypted::Config::create(
                        &password,
                        fusion_custody::encrypted::InnerConfig::Threshold(config),
                    )?)
                } else {
            ...
            (typ, InitSubCmd::ReEncrypt, true) => {
                ...
                let custody = match custody {
                    ...
                    CustodyConfig::SoftKms(spend_key) => {
                        ...
                        CustodyConfig::Encrypted(fusion_custody::encrypted::Config::create(
                            &password,
                            fusion_custody::encrypted::InnerConfig::SoftKms(spend_key),
                        )?)
                    }
                    CustodyConfig::Threshold(c) => {
                        ...
                        CustodyConfig::Encrypted(fusion_custody::encrypted::Config::create(
                            &password,
                            fusion_custody::encrypted::InnerConfig::Threshold(c),
                        )?)
                    }
                };
                (fvk, custody)
        ...
        let config = if let InitType::SpendKey = init_type {
            ...
            let config_path = home_dir.join(crate::CONFIG_FILE_NAME);
            let mut config = FcliConfig::load(config_path)?;
