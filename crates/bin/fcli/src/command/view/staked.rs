...
impl StakedCmd {
    ...
    pub async fn exec(
        ...
        fnsd_channel: Channel,
    ) -> Result<()> {
        let asset_cache = view_client.assets().await?;

        let mut client = StakeQueryServiceClient::new(fnsd_channel);
