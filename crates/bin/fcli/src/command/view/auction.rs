...
use fusion_proto::{core::component::auction::v1 as pb_auction, DomainType, Name};
...
pub struct AuctionCmd {
    ...
    /// If set, make the view server query an RPC and fcli render the full auction state
...
impl AuctionCmd {
    ...
    pub async fn exec(
        ...
        let auctions: Vec<(
            fusion_auction::auction::AuctionId,
            fusion_view::SpendableNoteRecord,
            u64,
            Option<pbjson_types::Any>,
            Vec<fusion_dex::lp::position::Position>,
        )> = view_client
        ...
        for (auction_id, _, local_seq, maybe_auction_state, positions) in auctions.into_iter() {
            if let Some(pb_auction_state) = maybe_auction_state {
                ...
                let position_ids: Vec<String> = positions
                    .into_iter()
                    .map(|lp: fusion_dex::lp::position::Position| format!("{}", lp.id()))
                    .collect();
