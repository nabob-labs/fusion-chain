...
use fusion_proto::{view::v1::GasPricesRequest, DomainType};
use fusion_stake::{
    validator,
...
#[derive(Debug, clap::Subcommand)]
pub enum VoteCmd {
    /// Cast a vote on a proposal in your capacity as a validator (see also: `fcli tx vote` for
    /// delegator voting).
    Cast {
        ...
        /// vote may be generated using the `fcli validator vote sign` command.
...
pub enum DefinitionCmd {
    ...
    Upload {
        ...
        ///
        /// This is useful for offline signing, e.g. in an airgap setup. The signature for the
        /// definition may be generated using the `fcli validator definition sign` command.
        #[clap(long)]
        signature: Option<String>,
        /// The selected fee tier to multiply the fee amount by.
        #[clap(short, long, default_value_t)]
        ...
        match self {
            ...
            ValidatorCmd::Definition(DefinitionCmd::Sign {
                ...
                if let Some(output_file) = signature_file {
                    ...
                    println!(
                        "To upload the definition, use the below command with the exact same definition file:\n\n  $ fcli validator definition upload --file {:?} --signature - < {:?}",
                        input_file_name,
                        output_file_name,
                    );
                } else {
                    println!(
                        "Signed validator definition #{} for {}\nTo upload the definition, use the below command with the exact same definition file:\n\n  $ fcli validator definition upload --file {:?} \\\n      --signature {}",
                        new_validator.sequence_number,
                        new_validator.identity_key,
                        input_file_name,
            ...
            ValidatorCmd::Definition(DefinitionCmd::Upload {
                ...
                let auth_sig = if let Some(signature) = signature {
                    ...
                    <Signature<SpendAuth> as fusion_proto::DomainType>::decode(
                        &URL_SAFE
                            .decode(signature)
            ...
            ValidatorCmd::Vote(VoteCmd::Sign {
                ...
                if let Some(signature_file) = signature_file {
                    ...
                    println!(
                        "To cast the vote, use the below command:\n\n  $ fcli validator vote cast {vote} --on {proposal} --reason {reason:?} --signature - < {signature_file:?}",
                    );
                } else {
                    println!(
                        "Signed validator vote {vote} on proposal #{proposal} by {identity_key}\nTo cast the vote, use the below command:\n\n  $ fcli validator vote cast {vote} --on {proposal} --reason {reason:?} \\\n      --signature {}",
                        URL_SAFE.encode(signature.encode_to_vec())
                    );
                }
            }
