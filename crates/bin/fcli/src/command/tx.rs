...
use fusion_keys::{keys::AddressIndex, Address};
use fusion_num::Amount;
use fusion_proto::core::app::v1::{
...
use fusion_shielded_pool::Ics20Withdrawal;
use fusion_stake::{
    rate::RateData,
    validator::{self},
};
...
use fusion_transaction::{gas::swap_claim_gas_cost, Transaction};
use fusion_view::{SpendableNoteRecord, ViewClient};
use fusion_wallet::plan::{self, Planner};
...
/// fcli splits apart the number of positions to close/withdraw
...
pub enum TxCmd {
    ...
    Auction(AuctionCmd),
    /// Send funds to a Fusion address.
    #[clap(display_order = 100)]
    Send {
        /// The destination address to send funds to.
        #[clap(long, display_order = 100)]
        to: String,
        /// The amounts to send, written as typed values 1.87fusion, 12cubes, etc.
        values: Vec<String>,
    ...
    DelegateMany {
        ...
        ///
        /// The amount is in UM, not ufusion.
        #[clap(long, display_order = 100)]
        csv_path: String,
        /// Only spend funds originally received by the given account.
        #[clap(long, default_value = "0", display_order = 300)]
    ...
    Swap {
        /// The input amount to swap, written as a typed value 1.87fusion, 12cubes, etc.
        input: String,
    ...
    /// Vote on a governance proposal in your role as a delegator (see also: `fcli validator vote`).
    ...
    ///
    /// Since Fusion transactions reveal their arity (how many spends,
    ...
    /// Perform an ICS-20 withdrawal, moving funds from the Fusion chain
    /// to a counterparty chain.
    ///
    /// For a withdrawal to be processed on the counterparty, IBC packets must be relayed between
    /// the two chains. Relaying is out of scope for the `fcli` tool.
    ...
    Withdraw {
        ...
        #[clap(long)]
        to: String,
        /// The value to withdraw, eg "1000ufusion"
        value: String,
        /// The IBC channel on the primary Fusion chain to use for performing the withdrawal.
        /// This channel must already exist, as configured by a relayer client.
        /// You can search for channels via e.g. `fcli query ibc channel transfer 0`.
        #[clap(long)]
        channel: u64,
        ...
        /// Only withdraw funds from the specified wallet id within Fusion.
    ...
    RegisterForwardingAccount {
        ...
        #[clap(long)]
        channel: String,
        /// The Fusion address or address index to receive forwarded funds.
        #[clap(long)]
    ...
    pub async fn exec(&self, app: &mut App) -> Result<()> {
        ...
        match self {
            ...
            TxCmd::Delegate {
                ...
                let mut stake_client = StakeQueryServiceClient::new(app.fnsd_channel().await?);
                ...
                let mut sct_client = SctQueryServiceClient::new(app.fnsd_channel().await?);
                ...
                let plan = planner
                    ...
                    .plan(app.view(), AddressIndex::new(*source))
                    .await
                    .context("can't plan delegation, try running fcli tx sweep and try again")?;
            ...
            TxCmd::DelegateMany {
                ...
                for result in reader.records() {
                    ...
                    let typed_amount_str = format!("{}fusion", &record[1]);
            ...
            TxCmd::UndelegateClaim { fee_tier } => {
                let channel = app.fnsd_channel().await?;
            ...
            TxCmd::Proposal(ProposalCmd::Template { file, kind }) => {
                ...
                let mut client = GovernanceQueryServiceClient::new(app.fnsd_channel().await?);
                ...
                let timeout_height = match timeout_height {
                    ...
                    None => {
                        ...
                        let mut ibc_channel_client =
                            IbcChannelQueryClient::new(app.fnsd_channel().await?);
                        ...
                        let mut ibc_connection_client =
                            IbcConnectionQueryClient::new(app.fnsd_channel().await?);
                        ...
                        let mut ibc_client_client =
                            IbcClientQueryClient::new(app.fnsd_channel().await?);
            ...
            TxCmd::Position(PositionCmd::WithdrawAll {
                ...
                let mut client = DexQueryServiceClient::new(app.fnsd_channel().await?);
            ...
            TxCmd::RegisterForwardingAccount {
                ...
                println!(
                    "registering Noble forwarding account with address {} to forward to Fusion address {}...",
