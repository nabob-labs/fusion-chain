...
use fusion_asset::asset::Id;
use fusion_asset::asset::Metadata;
use fusion_asset::Value;
use fusion_asset::ValueView;
use fusion_dex::swap::SwapView;
use fusion_dex::swap_claim::SwapClaimView;
use fusion_fee::Fee;
use fusion_keys::AddressView;
use fusion_num::Amount;
use fusion_shielded_pool::SpendView;
use fusion_transaction::view::action_view::OutputView;
use fusion_transaction::TransactionView;
...
impl TransactionViewExt for TransactionView {
    fn render_terminal(&self) {
        ...
        if let Some(memo_view) = &self.body_view.memo_view {
            match memo_view {
                fusion_transaction::MemoView::Visible {
                    plaintext,
                    ciphertext: _,
                } => {
                ...
                fusion_transaction::MemoView::Opaque { ciphertext } => {
        ...
        for action_view in &self.body_view.action_views {
            ...
            let row = match action_view {
                fusion_transaction::ActionView::Spend(spend) => {
                    match spend {
                ...
                fusion_transaction::ActionView::Output(output) => {
                    match output {
                        OutputView::Visible {
                            output: _,
                            note,
                ...
                fusion_transaction::ActionView::Swap(swap) => {
                ...
                fusion_transaction::ActionView::SwapClaim(swap_claim) => {
                    match swap_claim {
                ...
                fusion_transaction::ActionView::Ics20Withdrawal(withdrawal) => {
                ...
                fusion_transaction::ActionView::PositionOpen(position_open) => {
                ...
                fusion_transaction::ActionView::PositionClose(_) => {
                    ["Close Liquitity Position", ""]
                }
                fusion_transaction::ActionView::PositionWithdraw(_) => {
                    ["Withdraw Liquitity Position", ""]
                }
                fusion_transaction::ActionView::ProposalDepositClaim(
                ...
                fusion_transaction::ActionView::ProposalSubmit(proposal_submit) => {
                    action = format!(
                ...
                fusion_transaction::ActionView::ProposalWithdraw(proposal_withdraw) => {
                    action = format!(
                ...
                fusion_transaction::ActionView::IbcRelay(_) => ["IBC Relay", ""],
                fusion_transaction::ActionView::DelegatorVote(_) => ["Delegator Vote", ""],
                fusion_transaction::ActionView::ValidatorDefinition(_) => {
                    ["Upload Validator Definition", ""]
                }
                fusion_transaction::ActionView::ValidatorVote(_) => ["Validator Vote", ""],
                fusion_transaction::ActionView::CommunityPoolDeposit(_) => {
                    ["Community Pool Deposit", ""]
                }
                fusion_transaction::ActionView::CommunityPoolSpend(_) => {
                    ["Community Pool Spend", ""]
                }
                fusion_transaction::ActionView::CommunityPoolOutput(_) => {
                    ["Community Pool Output", ""]
                }
                fusion_transaction::ActionView::Delegate(_) => ["Delegation", ""],
                fusion_transaction::ActionView::Undelegate(_) => ["Undelegation", ""],
                fusion_transaction::ActionView::UndelegateClaim(_) => {
                    ["Undelegation Claim", ""]
                }
                fusion_transaction::ActionView::ActionDutchAuctionSchedule(x) => {
                ...
                fusion_transaction::ActionView::ActionDutchAuctionEnd(x) => {
                    action = format!("{}", x.auction_id);
                    ["Dutch Auction End", &action]
                }
                fusion_transaction::ActionView::ActionDutchAuctionWithdraw(x) => {
                    let inside = x
                        .reserves
                        .iter()
