...
use fusion_stake::GovernanceKey;
...
impl FcliConfig {
    pub fn load<P: AsRef<Path> + std::fmt::Display>(path: P) -> Result<Self> {
        let contents = std::fs::read_to_string(&path).context(format!(
            "fcli config file not found: {}. hint: run 'fcli init' to create new keys",
            &path
        ))?;
    ...
    fn toml_config() {
        let config = FcliConfig {
            ...
            custody: CustodyConfig::SoftKms(SoftKmsConfig::from(
                fusion_keys::test_keys::SPEND_KEY.clone(),
            )),
