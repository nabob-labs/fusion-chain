...
use clap::Parser;
use fusion_custody::{null_kms::NullKms, soft_kms::SoftKms};
...
use fusion_view::ViewServer;
use std::io::IsTerminal as _;
use tracing_subscriber::EnvFilter;
use url::Url;

#[derive(Debug, Parser)]
#[clap(name = "fcli", about = "The Fusion command-line interface.", version)]
pub struct Opt {
    #[clap(subcommand)]
    pub cmd: Command,
    /// The home directory used to store configuration and data.
    #[clap(long, default_value_t = default_home(), env = "FUSION_FCLI_HOME")]
    pub home: Utf8PathBuf,
    /// Override the GRPC URL that will be used to connect to a fullnode.
    ///
    /// By default, this URL is provided by fcli's config. See `fcli init` for more information.
...
impl Opt {
    ...
    pub fn load_config(&self) -> Result<FcliConfig> {
        let path = self.home.join(crate::CONFIG_FILE_NAME);
        let mut config = FcliConfig::load(path)?;
    ...
    pub async fn into_app(self) -> Result<(App, Command)> {
        ...
        let custody = match &config.custody {
            ...
            CustodyConfig::Threshold(config) => {
                ...
                let threshold_kms = fusion_custody::threshold::Threshold::new(
                    config.clone(),
                    ActualTerminal {
                        fvk: Some(fvk.clone()),
                    },
            ...
            CustodyConfig::Encrypted(config) => {
                ...
                let encrypted_kms = fusion_custody::encrypted::Encrypted::new(
                    config.clone(),
                    ActualTerminal {
                        fvk: Some(fvk.clone()),
                    },
