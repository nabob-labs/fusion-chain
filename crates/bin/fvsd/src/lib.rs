...
use fusion_custody::policy::{AuthPolicy, PreAuthorizationPolicy};
use fusion_custody::soft_kms::{self, SoftKms};
...
use fusion_view::{Storage, ViewServer};
...
pub struct Opt {
    ...
    #[clap(subcommand)]
    pub cmd: Command,
    /// The path used to store fvsd state and config files.
    #[clap(long, default_value_t = default_home(), env = "FUSION_FVSD_HOME")]
    pub home: Utf8PathBuf,
}

#[derive(Debug, clap::Subcommand)]
pub enum Command {
    /// Generate configs for `fvsd` in view or custody mode.
    Init {
    ...
    /// Start running `fvsd`.
    Start {},
    /// Delete `fvsd` storage to reset local state.
    Reset {},
}
        ...
        match &opt.cmd {
            Command::Reset {} => {
                if opt.sqlite_path().exists() {
                    ...
                    println!("No local storage at: {:?} (have you started fvsd, so it would have data to store?)", opt.sqlite_path());
                }
            ...
            Command::Init {
                ...
                let seed_phrase = match custody {
                    ...
                    Some(seed_phrase) => {
                        ...
                        if seed_phrase == "-" {
                            println!("Enter your seed phrase to enable fvsd custody mode: ");
            ...
            Command::Start {} => {
                let config = FvsdConfig::load(opt.config_path()).context(
                    "Failed to load fvsd config file. Have you run `fvsd init` with a FVK?",
                )?;

                tracing::info!(?opt.home, ?config.bind_addr, %config.grpc_url, "starting fvsd");
                let storage = opt
                    .load_or_init_sqlite(&config.full_viewing_key, &config.grpc_url)
                    .await?;

                let proxy_channel = ViewServer::get_fnsd_channel(config.grpc_url.clone()).await?;
                    ...
                    .add_service(tonic_web::enable(
                        tonic_reflection::server::Builder::configure()
                            .register_encoded_file_descriptor_set(
                                fusion_proto::FILE_DESCRIPTOR_SET,
                            )
                            .build_v1()
