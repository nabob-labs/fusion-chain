...
use fusion_proto::{fusion::core::asset::v1 as pb, DomainType};
use regex::Regex;
...
impl FromStr for Value {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        if let Some(captures) = asset_id_re.captures(s) {
            ...
            Err(anyhow::anyhow!(
                "could not parse {} as a value; provide both a numeric value and denomination, e.g. 1fusion",
                s
            ))
        }
    }
...
mod tests {
    ...
    fn format_picks_best_unit() {
        ...
        let v3: Value = "4000000ufusion".parse().unwrap();

        assert_eq!(v1.format(&cache), "999ufusion");
        assert_eq!(v2.format(&cache), "1mfusion");
        assert_eq!(v3.format(&cache), "4fusion");
    }
}
