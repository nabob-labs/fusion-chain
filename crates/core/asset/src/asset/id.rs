...
use fusion_proto::{fusion::core::asset::v1 as pb, serializers::bech32str, DomainType};
...
/// trace.  Instead, Fusion hashes to a field element, so that asset IDs can
...
pub static VALUE_GENERATOR_DOMAIN_SEP: Lazy<Fq> = Lazy::new(|| {
    Fq::from_le_bytes_mod_order(blake2b_simd::blake2b(b"fusion.value.generator").as_bytes())
});
...
mod tests {
    ...
    fn asset_id_encoding() {
        let id = Id::from_raw_denom("ufusion");
        ...
        let proto3 = pb::AssetId {
            alt_base_denom: "ufusion".to_owned(),
            ..Default::default()
        }
        .encode_to_vec();
