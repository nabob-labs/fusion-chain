...
pub static REGISTRY: Lazy<Registry> = Lazy::new(|| {
    Builder::default()
        ...
        .add_asset(
            ...
            // and VALIDATOR_IDENTITY_BECH32_PREFIX in the fusion-stake crate
            // TODO: this doesn't restrict the length of the bech32 encoding
            "^udelegation_(?P<data>fusionvalid1[a-zA-HJ-NP-Z0-9]+)$",
            &[
                "^delegation_(?P<data>fusionvalid1[a-zA-HJ-NP-Z0-9]+)$",
                "^mdelegation_(?P<data>fusionvalid1[a-zA-HJ-NP-Z0-9]+)$",
            ],
            (|data: &str| {
                assert!(!data.is_empty());
                denom_metadata::Inner::new(
        ...
        .add_asset(
            ...
            // and VALIDATOR_IDENTITY_BECH32_PREFIX in the fusion-stake crate
            // TODO: this doesn't restrict the length of the bech32 encoding
            "^uunbonding_(?P<data>start_at_(?P<start>[0-9]+)_(?P<validator>fusionvalid1[a-zA-HJ-NP-Z0-9]+))$",
            &[
                "^unbonding_(?P<data>start_at_(?P<start>[0-9]+)_(?P<validator>fusionvalid1[a-zA-HJ-NP-Z0-9]+))$",
                "^munbonding_(?P<data>start_at_(?P<start>[0-9]+)_(?P<validator>fusionvalid1[a-zA-HJ-NP-Z0-9]+))$",
            ],
            (|data: &str| {
                assert!(!data.is_empty());
                denom_metadata::Inner::new(
