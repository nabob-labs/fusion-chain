...
use fusion_proto::{fusion::core::asset::v1 as pb, view::v1::AssetsResponse, DomainType};
use regex::Regex;
...
impl From<&Inner> for pb::Metadata {
    fn from(inner: &Inner) -> Self {
        pb::Metadata {
            ...
            fusion_asset_id: Some(inner.id.into()),
...
impl TryFrom<pb::Metadata> for Inner {
    ...
    fn try_from(value: pb::Metadata) -> Result<Self, Self::Error> {
        ...
        if let Some(supplied_id) = value.fusion_asset_id {
            let supplied_id = Id::try_from(supplied_id)?;
            ensure!(
                id == supplied_id,
                "denom metadata has mismatched fusion asset ID"
            );
        }
...
mod tests {
    ...
    fn encoding_round_trip_succeeds() {
        let metadata = super::Metadata::try_from("ufusion").unwrap();
    ...
    fn changing_asset_id_without_changing_denom_fails_decoding() {
        let mut metadata = super::Metadata::try_from("ufusion").unwrap();
