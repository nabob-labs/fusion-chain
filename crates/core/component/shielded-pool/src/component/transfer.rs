...
use fusion_ibc::component::ChannelStateReadExt;
use fusion_keys::Address;
use fusion_num::Amount;
use fusion_proto::{
    fusion::core::component::ibc::v1::FungibleTokenPacketData, DomainType as _, StateReadProto,
    StateWriteProto,
};
use fusion_sct::CommitmentSource;

use fusion_ibc::component::{
...
// that originates from Fusion) or a bridged token (one that was sent into fusion from
// IBC).
//
...
pub trait Ics20TransferWriteExt: StateWrite {
    async fn withdrawal_execute(&mut self, withdrawal: &Ics20Withdrawal) -> Result<()> {
        ...
        if !withdrawal.denom.starts_with(&prefix) {
            ...
            //
            // for assets not originating from Fusion, never transfer out more tokens than were
...
impl AppHandlerCheck for Ics20Transfer {
    ...
    async fn timeout_packet_check<S: StateRead>(state: S, msg: &MsgTimeout) -> Result<()> {
        ...
        if is_source(&msg.packet.port_on_a, &msg.packet.chan_on_a, &denom, true) {
            ...
            let amount_fusion: Amount = packet_data.amount.try_into()?;
            if value_balance < amount_fusion {
...
async fn refund_tokens<S: StateWrite>(
    ...
    // other chain but was sent from fusion. so, the fusion refund receiver address is the
    // sender
