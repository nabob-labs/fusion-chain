...
use fusion_proto::{fusion::core::component::shielded_pool::v1 as pb, DomainType};
...
use fusion_proof_params::{DummyWitness, VerifyingKeyExt, GROTH16_PROOF_LENGTH_BYTES};
use fusion_sct::{Nullifier, NullifierVar};
use tap::Tap;
...
fn check_satisfaction(public: &SpendProofPublic, private: &SpendProofPrivate) -> Result<()> {
    use fusion_keys::keys::FullViewingKey;
...
impl DummyWitness for SpendCircuit {
    fn with_dummy_witness() -> Self {
        ...
        let note = Note::from_parts(
            address,
            Value::from_str("1ufusion").expect("valid value"),
            Rseed([1u8; 32]),
        )
...
mod tests {
    ...
    use fusion_proof_params::generate_prepared_test_parameters;
