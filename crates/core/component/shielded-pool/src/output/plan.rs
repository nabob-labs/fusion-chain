...
use fusion_asset::{Balance, Value, STAKING_TOKEN_ASSET_ID};
use fusion_keys::{
    keys::{IncomingViewingKey, OutgoingViewingKey},
    symmetric::WrappedMemoKey,
    Address, PayloadKey,
};
use fusion_proto::{core::component::shielded_pool::v1 as pb, DomainType};
...
impl OutputPlan {
    ...
    pub fn output_proof(&self) -> OutputProof {
        ...
        OutputProof::prove(
            ...
            &fusion_proof_params::OUTPUT_PROOF_PROVING_KEY,
            OutputProofPublic {
                balance_commitment,
                note_commitment,
            },
...
mod test {
    ...
    use fusion_proof_params::OUTPUT_PROOF_VERIFICATION_KEY;
    use rand_core::OsRng;

    #[test]
    /// Check that a valid output proof passes the `fusion_crypto` integrity checks successfully.
    ...
    fn check_output_proof_verification() {
        ...
        let value: Value = "1234.02fusion".parse().unwrap();
        let dest_address = "fusion1rqcd3hfvkvc04c4c9vc0ac87lh4y0z8l28k4xp6d0cnd5jc6f6k0neuzp6zdwtpwyfpswtdzv9jzqtpjn5t6wh96pfx3flq2dhqgc42u7c06kj57dl39w2xm6tg0wh4zc8kjjk".parse().unwrap();
