...
use fusion_proto::core::component::fee::v1 as pb;
use fusion_proto::state::StateWriteProto as _;
...
pub trait FeePay: StateWrite {
    ...
    async fn pay_fee(&mut self, gas_used: Gas, fee: Fee) -> Result<()> {
        let current_gas_prices = if fee.asset_id() == *fusion_asset::STAKING_TOKEN_ASSET_ID {
            self.get_gas_prices()
                .await
