...
pub trait StateWriteExt: StateWrite + StateReadExt {
    ...
    fn put_verified_heights(&mut self, client_id: &ClientId, verified_heights: VerifiedHeights) {
        self.put(
            format!(
                // NOTE: this is an implementation detail of the Fusion ICS2 implementation, so
                // it's not in the same path namespace.
                "fusion_verified_heights/{client_id}/verified_heights"
            ),
            verified_heights,
        );
    }
    ...
    fn put_fusion_sdk_consensus_state(
        ...
        // it's not in the same path namespace.
        self.put(
            format!("fusion_consensus_states/{height}"),
            consensus_state,
        );
    }
}
...
pub trait StateReadExt: StateRead {
    ...
    async fn get_client_status(
        ...
        // We don't have a similar allowlist in Fusion, so we skip that check.
    ...
    async fn get_fusion_sdk_consensus_state(
        ...
        // NOTE: this is an implementation detail of the Fusion ICS2 implementation, so
        // it's not in the same path namespace.
        self.get(&format!("fusion_consensus_states/{height}"))
            .await?
            .ok_or_else(|| {
                anyhow::anyhow!("fusion consensus state not found for height {height}")
            })
    }
...
mod tests {
    ...
    use fusion_sct::component::clock::{EpochManager as _, EpochRead};
    use std::str::FromStr;
    use tendermint::Time;
