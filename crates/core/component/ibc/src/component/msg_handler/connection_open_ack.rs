...
impl MsgHandler for MsgConnectionOpenAck {
    ...
    async fn try_execute<S: StateWrite, AH, HI: HostInterface>(&self, mut state: S) -> Result<()> {
        ...
        // committed a _valid_ Fusion consensus state, that the counterparty chain has committed
        // the expected Connection to its state (in the TRYOPEN state) with the expected version,
        // and that the counterparty has committed a correct Fusion client state to its state.
        //
        // Here we are Chain A.
        // CHAINS:          (A, B)
        ...
        fusion_sdk_client_state_is_well_formed::<&S, HI>(&state, self).await?;
        ...
        let expected_consensus = state
            .get_fusion_sdk_consensus_state(self.consensus_height_of_a_on_b)
            .await?;
...
async fn fusion_sdk_client_state_is_well_formed<S: StateRead, HI: HostInterface>(
    ...
    validate_fusion_sdk_client_state(msg.client_state_of_a_on_b.clone(), &chain_id, height)?;
