...
impl MsgHandler for MsgConnectionOpenAck {
    ...
    async fn try_execute<S: StateWrite, AH, HI: HostInterface>(&self, mut state: S) -> Result<()> {
        ...
        fusion_client_state_is_well_formed::<&S, HI>(&state, self).await?;
        ...
        let expected_consensus = state
            .get_fusion_consensus_state(self.consensus_height_of_a_on_b)
            .await?;
...
async fn fusion_client_state_is_well_formed<S: StateRead, HI: HostInterface>(
    ...
    validate_fusion_client_state(msg.client_state_of_a_on_b.clone(), &chain_id, height)?;
