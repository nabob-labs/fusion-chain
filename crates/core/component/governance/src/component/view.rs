...
use fusion_asset::{asset, Value, STAKING_TOKEN_DENOM};
use fusion_ibc::component::ClientStateReadExt as _;
use fusion_ibc::component::ClientStateWriteExt as _;
...
use fusion_shielded_pool::component::AssetRegistryRead;
use fusion_stake::{
...
use fusion_stake::{rate::RateData, validator};
...
#[async_trait]
pub trait StateReadExt: StateRead + fusion_stake::StateReadExt {
...
impl<T: StateRead + fusion_stake::StateReadExt + ?Sized> StateReadExt for T {}

#[async_trait]
pub trait StateWriteExt: StateWrite + fusion_ibc::component::ConnectionStateWriteExt {
    ...
    async fn new_proposal(&mut self, proposal: &Proposal) -> Result<u64> {
        ...
        while let Some(identity_key) = validator_identity_stream.next().await {
            ...
            let power: fusion_proto::state::future::DomainFuture<
                Amount,
