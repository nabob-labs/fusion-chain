...
use fusion_fee::Fee;
use fusion_proto::{
    core::keys::v1 as pb_keys, fusion::core::component::dex::v1 as pb, DomainType,
};
...
use fusion_asset::{asset, Value, ValueVar};
use fusion_keys::{keys::OutgoingViewingKey, Address, AddressVar, PayloadKey};
use fusion_num::{Amount, AmountVar};
use fusion_shielded_pool::{Note, Rseed};
use fusion_tct::r1cs::StateCommitmentVar;
...
pub static OUTPUT_1_BLINDING_DOMAIN_SEPARATOR: Lazy<Fq> = Lazy::new(|| {
    Fq::from_le_bytes_mod_order(
        blake2b_simd::blake2b(b"fusion.swapclaim.output1.blinding").as_bytes(),
    )
});
pub static OUTPUT_2_BLINDING_DOMAIN_SEPARATOR: Lazy<Fq> = Lazy::new(|| {
    Fq::from_le_bytes_mod_order(
        blake2b_simd::blake2b(b"fusion.swapclaim.output2.blinding").as_bytes(),
    )
});
