...
use fusion_num::{fixpoint::U128x128, Amount};
use rand_core::OsRng;
use std::sync::Arc;
...
async fn path_search_basic() {
    ...
    // Try routing from "gm" to "fusion".
    let gm = asset::Cache::with_known_assets().get_unit("gm").unwrap();

    let fusion = asset::Cache::with_known_assets()
        .get_unit("fusion")
        .unwrap();

    let mut routing_params = state.routing_params().await.unwrap();
    tracing::info!(src = %gm, dst = %fusion, "searching for path");
    let (_path, _spill) = state
        .path_search(gm.id(), fusion.id(), routing_params.clone())
        .await
        .unwrap();

    // Now try routing from "fusion" to "fusion".
    tracing::info!(src = %fusion, dst = %fusion, "searching for path");
    routing_params.max_hops = 8;
    let (_path, _spill) = state
        .path_search(fusion.id(), fusion.id(), routing_params)
        .await
        .unwrap();
}
...
async fn path_extension_basic() {
    ...
    // Extending directly to "fusion" should fail as there
    // are no positions from GN <-> Fusion.
    let fusion = asset::Cache::with_known_assets()
        .get_unit("fusion")
        .unwrap();
    ...
    assert!(
        path.fork()
            ...
            .expect("extend_to failed")
            .is_none(),
        "path to fusion should not exist"
    );
    ...
    let path = path
        ...
        .await
        .expect("extend_to failed")
        .expect("path to fusion not found");

    assert_eq!(path.end(), &fusion.id(), "path ends on fusion");
    assert_eq!(path.start, gm.id(), "path starts on gm");
...
async fn create_test_positions_basic<S: StateWrite>(s: &mut S, misprice: bool) {
    ...
    tracing::debug!(id = ?fusion.id(), unit = %fusion);
    tracing::debug!(id = ?pusd.id(), unit = %pusd);

    // `pusd` is treated as a numeraire, with gm:pusd, gn:pusd, and fusion:pusd pairs with different prices.
    // some of the `gn:pusd` positions will be mispriced so we can exercise arbitrage and cycle resolution.
    // routing through `pusd` will let us test synthetic liquidity on the `gm:fusion` and `gn:fusion` pairs (which will have
    // no direct positions).
    // ┌──────┐
    // │      │         ┌─────┐
    // │  gm  │◀───┐    │ gn  │
    ...
    // fusion <-> pusd
    let pen_pusd_pair = DirectedTradingPair::new(fusion.id(), pusd.id());
    ...
    // 1bps fee from Fusion <-> PUSD at 1:10
    let position_7 = Position::new(
        ...
        Amount::from(1u64) * pusd.unit_amount(),
        Amount::from(10u64) * fusion.unit_amount(),
        Reserves {
    ...
    // than 1 person providing liquidity on fusion.
...
async fn position_get_best_price() -> anyhow::Result<()> {
    ...
    let pair = DirectedTradingPair::new(gn.id(), fusion.id());
    let position_1 = Position::new(
        OsRng,
        pair,
        1,
...
async fn fill_route_constraint_stacked() -> anyhow::Result<()> {
    ...
    /*
            ------------------------------------------------------------------------------------------------------------
            |       Pair 1: gm <> gn       |       Pair 2: gn <> fusion        |       Pair 3: fusion <> pusd      |
            ------------------------------------------------------------------------------------------------------------
            | ^-bids---------asks-v        |    ^-bids---------asks-v             |       ^-bids---------asks-v        |
            |           3gm@2              |           1gn@2                      |           1fusion@10000          |
            |           2gm@1              |          50gn@1                      |            1fusion@3100          |
            |                              |          50gn@1                      |          198fusion@3000          |
            |                              |          50gn@1                      |           1fusion@2500           |
            |                              |                                      |           1fusion@2000           |
            ------------------------------------------------------------------------------------------------------------
            * marginal price
    ...
    */
    ...
    let pair_1 = DirectedUnitPair::new(gm.clone(), gn.clone());
    let pair_2 = DirectedUnitPair::new(gn.clone(), fusion.clone());
    let pair_3 = DirectedUnitPair::new(fusion.clone(), pusd.clone());
    ...
    let route = vec![gn.id(), fusion.id(), pusd.id()];
...
async fn fill_route_constraint_1() -> anyhow::Result<()> {
    ...
    /*
            ...
            ------------------------------------------------------------------------------------------------------------
            | ^-bids---------asks-v        |    ^-bids---------asks-v             |       ^-bids---------asks-v        |
            |         200gm@1              |          50gn@2                      |           1fusion@10000          |
            |                              |          50gn@2                      |            1fusion@3100          |
            |                              |          50gn@2                      |          198fusion@3000          |
            |                              |          50gn@2                      |           1fusion@2500           |
            |                              |                                      |           1fusion@2000           |
            ------------------------------------------------------------------------------------------------------------
            Delta_1 = 4gm
...
async fn fill_route_hit_spill_price() -> anyhow::Result<()> {
    ...
    */
    ...
    let spill_price = U128x128::ratio(valuation_gm, valuation_fusion).unwrap();
...
async fn simple_route() -> anyhow::Result<()> {
    ...
    let pair_1 = DirectedUnitPair::new(gn.clone(), fusion.clone());

    // Create a single 1:1 gn:fusion position (i.e. buy 1 gn at 1 fusion).
    ...
    let (path, _spill) = state
        .path_search(gn.id(), fusion.id(), routing_params)
        .await
        .unwrap();

    assert!(path.is_some(), "path exists between gn<->fusion");
    assert!(path.clone().unwrap().len() == 1, "path is of length 1");
    assert!(path.unwrap()[0] == fusion.id(), "path[0] is fusion");
...
async fn best_position_route_and_fill() -> anyhow::Result<()> {
    ...
    // Output data should have 1 fusion out and 1 gn in.
...
async fn multi_hop_route_and_fill() -> anyhow::Result<()> {
    ...
    let pair_gn_fusion = DirectedUnitPair::new(gn.clone(), fusion.clone());
    let pair_gm_gn = DirectedUnitPair::new(gm.clone(), gn.clone());
    let pair_gn_gm = DirectedUnitPair::new(gn.clone(), gm.clone());
    let pair_gm_fusion = DirectedUnitPair::new(gm.clone(), fusion.clone());

    // Create a 2:1 fusion:gm position (i.e. buy 20 gm at 2 fusion each).
    let buy_1 = create_buy_pq(
        pair_gm_fusion.clone(),
        5u64.into(),
        1u64.into(),
        2u64.into(),
        0u32,
    ...
    // Create a 2.1:1 fusion:gm position (i.e. buy 40 gm at 2.1 fusion each).
    ...
    // Create a 2.2:1 fusion:gm position (i.e. buy 160 gm at 2.2 fusion each).
    ...
    // Create a 1.9:1 fusion:gn position (i.e. buy 160 gn at 1.9 fusion each).
    let buy_5 = Position::new(
        OsRng,
        pair_gn_fusion.into_directed_trading_pair(),
        0u32,
        1000000u64.into(),
        1900000u64.into(),
        Reserves {
    ...
    // Now if we swap 1000gm into fusion, we should not get total execution, but we should
    // consume all fusion liquidity on the direct gm:fusion pairs, as well as route through the
    // gm:gn and gn:fusion pairs to obtain fusion.
    let routing_params = state.routing_params().await.unwrap();
    let (path, _spill) = state
        .path_search(gm.id(), fusion.id(), routing_params)
        .await
        .unwrap();

    assert!(path.is_some(), "path exists between gm<->fusion");
    assert!(path.unwrap()[0] == fusion.id(), "path[0] is fusion");

    let trading_pair = pair_gm_fusion.into_directed_trading_pair().into();
    ...
    // Output data should have 1 fusion out and 1000000 gm in.
    let output_data = state.output_data(0, trading_pair).await?.unwrap();
...
async fn fill_dust_route() -> anyhow::Result<()> {
    ...
    let route = vec![gn.id(), fusion.id()];

    let spill_price =
        (U128x128::from(1_000_000_000_000u64) * U128x128::from(fusion.unit_amount())).unwrap();
...
async fn fill_route_with_stacked_dust_constraint() -> anyhow::Result<()> {
    ...
    let pair_1 = DirectedUnitPair::new(gm.clone(), gn.clone());
    let pair_2 = DirectedUnitPair::new(gn.clone(), fusion.clone());
    let pair_3 = DirectedUnitPair::new(fusion.clone(), btc.clone());
    ...
    let route = vec![gn.id(), fusion.id(), btc.id(), pusd.id()];
...
async fn path_search_testnet_53_1_reproduction() -> anyhow::Result<()> {
    ...
    let s_a = SellOrder::parse_str("1gm@1fusion")
        .unwrap()
        .into_position(OsRng);
    let a_t = SellOrder::parse_str("1fusion@0.3gm")
        .unwrap()
        .into_position(OsRng);

    let s_b = SellOrder::parse_str("1gn@1fusion")
        .unwrap()
        .into_position(OsRng);
    let b_t = SellOrder::parse_str("1fusion@0.2gn")
        .unwrap()
        .into_position(OsRng);

    let s_c = SellOrder::parse_str("1test_usd@1fusion")
        .unwrap()
        .into_position(OsRng);
    let c_t = SellOrder::parse_str("1fusion@0.1test_usd")
        .unwrap()
        .into_position(OsRng);
    ...
    let cache = PathCache::begin(fusion.id(), state.fork());
    let mut cache_guard = cache.lock();
    let mut identity_path = cache_guard.0.get_mut(&fusion.id()).unwrap().path.fork();
    ...
    let Some(path_entry) = cache_guard.0.remove(&fusion.id()) else {
...
async fn path_search_commutative() -> anyhow::Result<()> {
    ...
    let a_t = SellOrder::parse_str("1fusion@0.9gm")
        .unwrap()
        .into_position(OsRng);

    let b_t = SellOrder::parse_str("1fusion@0.3gn")
        .unwrap()
        .into_position(OsRng);
    ...
    let c_t = SellOrder::parse_str("1fusion@0.99test_usd")
        .unwrap()
        .into_position(OsRng);
    ...
    let d_t = SellOrder::parse_str("1fusion@0.01test_atom")
        .unwrap()
        .into_position(OsRng);
