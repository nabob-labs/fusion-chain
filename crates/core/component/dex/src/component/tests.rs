...
async fn swap_execution_tests() -> anyhow::Result<()> {
    ...
    let buy_1 = create_buy(pair_gn_fusion.clone(), 1u64.into(), 1u64.into());
    state_tx.open_position(buy_1).await.unwrap();
    state_tx.apply();

    // Now we should be able to fill a 1:1 gn:fusion swap.
    let trading_pair = pair_gn_fusion.into_directed_trading_pair().into();
    ...
    // Swap execution should have a single trace consisting of `[1gn, 1fusion]`.
    ...
    assert_eq!(
        ...
        vec![vec![gn.value(1u32.into()), fusion.value(1u32.into()),]]
    );
    ...
    tracing::info!(fusion_id = ?fusion.id());
    ...
    state_tx
        ...
        .await
        .unwrap();
    // Buy 5 fusion at 1 gm each.
    state_tx
        .open_position(create_buy(
            DirectedUnitPair::new(fusion.clone(), gm.clone()),
            5u64.into(),
            1u64.into(),
        ))
        .await
        .unwrap();
    // Sell 1pusd at 5 fusion each.
    state_tx
        .open_position(create_sell(
            DirectedUnitPair::new(pusd.clone(), fusion.clone()),
            1u64.into(),
            5u64.into(),
        ))
        .await
    ...
    // Now we should be able to fill a 10fusion into 25gn swap.
    let trading_pair = pair_gn_fusion.into_directed_trading_pair().into();
    ...
    // Add the amount of each asset being swapped to the batch swap flow.
    swap_flow.0 += Amount::from(10u32) * fusion.unit_amount();
    ...
    assert_eq!(
        output_data,
        BatchSwapOutputData {
            delta_1: fusion.value(10u32.into()).amount,
            delta_2: 0u32.into(),
            lambda_1: 0u32.into(),
...
async fn basic_cycle_arb() -> anyhow::Result<()> {
    ...
    // Sell 10 gn at 1 fusion each.
    state_tx
        .open_position(create_sell(
            DirectedUnitPair::new(gn.clone(), fusion.clone()),
            10u64.into(),
            1u64.into(),
        ))
        .await
    ...
    // Now we should be able to arb 10fusion => 10gn => 20gm => 20fusion.
    let routing_params = RoutingParams {
        max_hops: 4 + 2,
        price_limit: Some(1u64.into()),
        fixed_candidates: Arc::new(vec![fusion.id(), gm.id(), gn.id()]),
    };
    state.arbitrage(fusion.id(), routing_params).await?;

    let arb_execution = state.arb_execution(0).await?.expect("arb was performed");
    assert_eq!(
        arb_execution.traces,
        vec![vec![
            fusion.value(10u32.into()),
            gn.value(10u32.into()),
            gm.value(20u32.into()),
            fusion.value(20u32.into()),
        ],]
    );
...
async fn reproduce_arbitrage_loop_testnet_53() -> anyhow::Result<()> {
    ...
    let fusion_usd = DirectedUnitPair::new(fusion.clone(), test_usd.clone());

    /*
     * INITIAL STATE:
     * Position A: Seeks to buy 1 fusion for 110 test_usd
     * Position B: Seeks to buy 1 fusion for 100 test_usd
     * Position C: Seeks to sell 10 fusion for 100 test_usd
     *
     * The arbitrage logic should detect that it can sell 1 fusion for 110 test_usd,
     * (Position A), and buy it back for 100 test_usd (Position B), and thus make a profit
     * of ~0.1 fusion. The execution price on the cycle is 0.909 fusion/test_usd/fusion, and the
     * spill price is 1 fusion/test_usd/fusion.
     *
     ...
     * Position A: Seeks to sell 1 fusion for 110 test_usd
     * Position B: Seeks to sell 1 fusion for 100 test_usd
     * Position C: Seeks to sell 10 fusion for 100 test_usd
     *
     *
     * Notice that Position A and Position B are now both seeking to sell fusion for usd,
     * because they were filled previously. The execution price on fusion -> usd -> fusion
    ...
    let mut buy_1 = create_buy(fusion_usd.clone(), 1u64.into(), 110u64.into());
    buy_1.nonce = [1; 32];

    let mut buy_2 = create_buy(fusion_usd.clone(), 1u64.into(), 100u64.into());
    buy_2.nonce = [2; 32];

    let mut sell_1 = create_sell(fusion_usd.clone(), 10u64.into(), 100u64.into());
    sell_1.nonce = [0; 32];
    ...
    let routing_params = RoutingParams {
        max_hops: 4 + 2,
        price_limit: Some(1u64.into()),
        fixed_candidates: Arc::new(vec![fusion.id(), test_usd.id()]),
    };

    let arb_profit = tokio::time::timeout(
        tokio::time::Duration::from_secs(2),
        state.arbitrage(fusion.id(), routing_params),
    )
    .await??;

    tracing::info!(profit = ?arb_profit, "the arbitrage logic has concluded!");
    // we should have made a profit of 0.01fusion, with precision loss of 0.000002fusion.
    let profit: Value = "0.099998fusion".parse().unwrap();
...
async fn check_routable_asset_ordering() -> anyhow::Result<()> {
    ...
    let fusion_usd = DirectedTradingPair::new(fusion.id(), test_usd.id());
    ...
    let fusion_gn = DirectedTradingPair::new(fusion.id(), gn.id());
    ...
    let fusion_btc = DirectedTradingPair::new(fusion.id(), test_btc.id());
    ...
    // Find routable assets starting at the Fusion asset.
    let routable_assets: Vec<_> = state
        .ordered_routable_assets(&fusion.id())
        .collect::<Vec<_>>()
        .await;
