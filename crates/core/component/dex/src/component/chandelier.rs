...
use fusion_num::fixpoint::U128x128;
use fusion_proto::{DomainType, StateReadProto, StateWriteProto};
use fusion_sct::component::clock::EpochRead as _;
...
mod tests {
    ...
    use fusion_asset::asset;
    use fusion_sct::{component::clock::EpochManager as _, epoch::Epoch};
    use tendermint::abci;
    ...
    async fn chandelier_basic() -> anyhow::Result<()> {
        ...
        // Create a single 1:2 gn:fusion position (i.e. buy 1 gn at 2 fusion).
        let mut state_tx = state.try_begin_transaction().unwrap();
        let buy_1 = create_buy(pair_gn_fusion.clone(), 1u64.into(), 2u64.into());
        ...
        assert!(
            state
                .get_candlestick(&pair_gn_fusion.into_directed_trading_pair(), 0u64)
                .await
                .unwrap()
                .is_some(),
        ...
        let mut state_tx = state.try_begin_transaction().unwrap();
        let buy_2 = create_buy(pair_gn_fusion.clone(), 1u64.into(), 1u64.into());
        state_tx.open_position(buy_2).await.unwrap();
        state_tx.apply();

        // A swap for gn -> fusion should first apply against the 1:2 position
        ...
        // Add the amount of each asset being swapped to the batch swap flow.
        swap_flow.0 += 0u32.into();
        // Swap 2 gn into fusion, meaning each position is filled.
        ...
        assert!(
            state
                .get_candlestick(&pair_gn_fusion.into_directed_trading_pair(), height)
                .await
                .unwrap()
                .is_some(),
