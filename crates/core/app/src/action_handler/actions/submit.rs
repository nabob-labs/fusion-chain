...
use fusion_asset::STAKING_TOKEN_DENOM;
use fusion_community_pool::component::StateReadExt as _;
...
use fusion_ibc::component::ClientStateReadExt;
use fusion_keys::keys::{FullViewingKey, NullifierKey};
use fusion_proto::{DomainType, StateWriteProto as _};
use fusion_sct::component::clock::EpochRead;
use fusion_sct::component::tree::SctRead;
use fusion_shielded_pool::component::AssetRegistry;
use fusion_transaction::{AuthorizationData, Transaction, TransactionPlan, WitnessData};
...
impl AppActionHandler for ProposalSubmit {
    ...
    async fn check_stateless(&self, _context: ()) -> Result<()> {
        ...
        use fusion_governance::ProposalPayload::*;
        match payload {
            ...
            CommunityPoolSpend { transaction_plan } => {
                ...
                use fusion_transaction::plan::ActionPlan::*;
...
/// The full viewing key used to construct transactions made by the Fusion Community Pool.
///
...
static COMMUNITY_POOL_FULL_VIEWING_KEY: Lazy<FullViewingKey> = Lazy::new(|| {
    ...
    let ak_hash_input =
        b"This hash input is used to form the `ak` component of the Fusion Community Pool's full viewing key.";
    let nk_hash_input =
        b"This hash input is used to form the `nk` component of the Fusion Community Pool's full viewing key.";
    ...
    .expect("fusion Community Pool FVK's `ak` must be a valid verification key by construction");
...
async fn build_community_pool_transaction(
    ...
    transaction_plan.build(
        ...
        &WitnessData {
            anchor: fusion_tct::Tree::new().root(),
