...
fn check_version(ctx: CheckContext, expected: u64, found: Option<u64>) -> anyhow::Result<()> {
    ...
    match ctx {
        CheckContext::Running => {
            ...
            write!(&mut error, "  found {} (fusion {})\n", found, found_name)?;
            write!(
                &mut error,
                "make sure you're running fusion {}",
                expected_name
            )?;
            Err(anyhow!(error))
        }
        CheckContext::Migration => {
            ...
            write!(
                &mut error,
                "this migration should be run with fusion {} instead",
...
pub async fn migrate_app_version<S: StateWriteProto>(s: &mut S, to: u64) -> anyhow::Result<()> {
    anyhow::ensure!(to > 1, "you can't migrate to the first fusion version!");
