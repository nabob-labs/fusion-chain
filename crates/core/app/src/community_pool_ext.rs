...
use fusion_governance::state_key;
...
// Note: These should live in `fusion-governance` in the `StateReadExt` and `StateWriteExt`
// traits, however that would result in a circular dependency since the below methods
// require use of `fusion-transaction::Transaction`, which has `fusion-governance` as a
// dependency.
#[async_trait]
pub trait CommunityPoolStateReadExt: StateRead + fusion_stake::StateReadExt {
...
impl<T: StateRead + fusion_stake::StateReadExt + ?Sized> CommunityPoolStateReadExt for T {}
