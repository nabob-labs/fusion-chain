...
use jmt::RootHash;
use fusion_auction::component::{Auction, StateReadExt as _, StateWriteExt as _};
use fusion_community_pool::component::{CommunityPool, StateWriteExt as _};
use fusion_community_pool::StateReadExt as _;
use fusion_compact_block::component::CompactBlockManager;
use fusion_dex::component::StateReadExt as _;
use fusion_dex::component::{Dex, StateWriteExt as _};
use fusion_distributions::component::{Distributions, StateReadExt as _, StateWriteExt as _};
use fusion_fee::component::{FeeComponent, StateReadExt as _, StateWriteExt as _};
use fusion_funding::component::Funding;
use fusion_funding::component::{StateReadExt as _, StateWriteExt as _};
use fusion_governance::component::{Governance, StateReadExt as _, StateWriteExt as _};
use fusion_ibc::component::{Ibc, StateWriteExt as _};
use fusion_ibc::StateReadExt as _;
use fusion_proto::core::app::v1::TransactionsByHeightResponse;
use fusion_proto::DomainType;
use fusion_sct::component::clock::EpochRead;
use fusion_sct::component::sct::Sct;
use fusion_sct::component::{StateReadExt as _, StateWriteExt as _};
use fusion_sct::epoch::Epoch;
use fusion_shielded_pool::component::{ShieldedPool, StateReadExt as _, StateWriteExt as _};
use fusion_stake::component::{
    stake::ConsensusUpdateRead, Staking, StateReadExt as _, StateWriteExt as _,
};
use fusion_transaction::Transaction;
...
use crate::{CommunityPoolStateReadExt, FusionHost};
...
/// The Fusion application, written as a bundle of [`Component`]s.
///
...
impl App {
    ...
    pub async fn begin_block(&mut self, begin_block: &request::BeginBlock) -> Vec<abci::Event> {
        ...
        Ibc::begin_block::<FusionHost, StateDelta<Arc<StateDelta<cnidarium::Snapshot>>>>(
            &mut arc_state_tx,
            begin_block,
        )
        .await;
    ...
    pub async fn end_block(&mut self, end_block: &request::EndBlock) -> Vec<abci::Event> {
        ...
        if is_end_epoch || is_chain_upgrade {
            ...
            fusion_sct::component::clock::EpochManager::put_epoch_by_height(
                &mut state_tx,
                current_height + 1,
                Epoch {
...
pub trait StateReadExt: StateRead {
    ...
    async fn get_app_params(&self) -> Result<AppParameters> {
        ...
        let community_pool_params: fusion_community_pool::params::CommunityPoolParameters =
...
impl<
        T: StateRead
            + fusion_stake::StateReadExt
            + fusion_governance::component::StateReadExt
            + fusion_fee::component::StateReadExt
            + fusion_community_pool::component::StateReadExt
            + fusion_sct::component::clock::EpochRead
            + fusion_ibc::component::StateReadExt
            + fusion_distributions::component::StateReadExt
            + ?Sized,
    > StateReadExt for T
{
}
...
pub trait StateWriteExt: StateWrite {
    ...
    async fn put_block_transaction(
        &mut self,
        height: u64,
        transaction: fusion_proto::core::transaction::v1::Transaction,
    ) -> Result<()> {
