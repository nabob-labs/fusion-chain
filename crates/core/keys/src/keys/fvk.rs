...
use fusion_proto::{fusion::core::keys::v1 as pb, serializers::bech32str, DomainType};
...
impl FullViewingKey {
    ...
    pub fn from_components(ak: VerificationKey<SpendAuth>, nk: NullifierKey) -> Self {
        let ovk = {
            let hash_result = prf::expand(b"Fusioned_DeriOVK", &nk.0.to_bytes(), ak.as_ref());
            let mut ovk = [0; 32];
        ...
        let dk = {
            let hash_result = prf::expand(b"Fusioned_DerivDK", &nk.0.to_bytes(), ak.as_ref());
            let mut dk = [0; 16];
