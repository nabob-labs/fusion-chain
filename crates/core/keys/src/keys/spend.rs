...
use hmac::Hmac;
use pbkdf2::pbkdf2;
use fusion_proto::{fusion::core::keys::v1 as pb, DomainType};
...
impl From<SpendKeyBytes> for SpendKey {
    fn from(seed: SpendKeyBytes) -> Self {
        let ask = SigningKey::new_from_field(prf::expand_ff(b"Fusioned_ExpndSd", &seed.0, &[0; 1]));
        let nk = NullifierKey(prf::expand_ff(b"Fusioned_ExpndSd", &seed.0, &[1; 1]));
...
impl std::fmt::Display for SpendKey {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        use fusion_proto::serializers::bech32str;
