...
use anyhow::Context;
use fusion_auction::auction::AuctionNft;
use fusion_compact_block::CompactBlock;
use fusion_dex::lp::{position, LpNft};
use fusion_keys::FullViewingKey;
use fusion_proto::core::{
    app::v1::{
...
use fusion_sct::{CommitmentSource, Nullifier};
...
pub struct Worker {
    storage: Storage,
    sct: Arc<RwLock<fusion_tct::Tree>>,
            ...
            if !block.requires_scanning() {
                ...
                for transaction in &transactions {
                    for action in transaction.actions() {
                        match action {
                            fusion_transaction::Action::PositionOpen(position_open) => {
                            ...
                            fusion_transaction::Action::PositionClose(position_close) => {
                            ...
                            fusion_transaction::Action::PositionWithdraw(
                                position_withdraw,
                            ) => {
                            ...
                            fusion_transaction::Action::ActionDutchAuctionSchedule(
                                schedule_da,
                            ) => {
                            ...
                            fusion_transaction::Action::ActionDutchAuctionEnd(end_da) => {
                            ...
                            fusion_transaction::Action::ActionDutchAuctionWithdraw(
                                withdraw_da,
                            ) => {
...
async fn sct_divergence_check(
    ...
    let expected_root = fusion_tct::Root::decode(value.value.as_slice())?;
