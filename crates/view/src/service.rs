...
use fusion_auction::auction::dutch::actions::view::{
...
use fusion_asset::{asset, asset::Metadata, Value};
use fusion_dex::{
    lp::{
...
impl ViewServer {
    ...
    pub async fn new(storage: Storage, node: Url) -> anyhow::Result<Self> {
        ...
        let channel = Self::get_fnsd_channel(node.clone()).await?;
    ...
    pub async fn get_fnsd_channel(node: Url) -> anyhow::Result<Channel> {
        let endpoint = get_fnsd_endpoint(node).await?;
        let span = tracing::error_span!(parent: None, "view");
                ...
                let nullifier = if await_detection {
                    ...
                    transaction
                        ...
                        .filter_map(|action| match action {
                            fusion_transaction::Action::Spend(spend) => Some(spend.body.nullifier),
                            /*
                            fusion_transaction::Action::SwapClaim(swap_claim) => {
...
impl ViewService for ViewServer {
    ...
    async fn transaction_info_by_hash(
        ...
        for action_view in min_view.action_views() {
            ...
            use fusion_transaction::view::action_view::{
