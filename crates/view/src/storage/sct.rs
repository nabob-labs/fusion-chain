...
mod test {
    ...
    use fusion_tct::{StateCommitment, Witness};
    ...
    fn tree_store_spot_check() {
        ...
        let deserialized = fusion_tct::Tree::from_reader(&mut store).unwrap();
        assert_eq!(deserialized, fusion_tct::Tree::new());

        // Make some kind of tree:
        let mut tree = fusion_tct::Tree::new();
