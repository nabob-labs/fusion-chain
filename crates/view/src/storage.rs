...
use fusion_app::params::AppParameters;
use fusion_asset::{asset, asset::Id, asset::Metadata, Value};
use fusion_dex::{
    lp::position::{self, Position, State},
    TradingPair,
};
use fusion_fee::GasPrices;
use fusion_keys::{keys::AddressIndex, Address, FullViewingKey};
...
use fusion_shielded_pool::{fmd, note, Note, Rseed};
use fusion_stake::{DelegationToken, IdentityKey};
use fusion_tct::{self as tct, builder::epoch::Root};
...
impl Storage {
    ...
    pub async fn load(path: impl AsRef<Utf8Path>) -> anyhow::Result<Self> {
        ...
        spawn_blocking(move || {
            ...
            if actual_schema_hash != *SCHEMA_HASH {
                ...
                anyhow::bail!(
                    "can't load view database created by client version {} using client version {}: they have different schemata, so you need to reset your view database and resynchronize by running fcli view reset",
    ...
    pub async fn notes(
        ...
        include_spent: bool,
        asset_id: Option<asset::Id>,
        address_index: Option<fusion_keys::keys::AddressIndex>,
