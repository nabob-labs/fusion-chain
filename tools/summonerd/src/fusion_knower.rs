...
use fusion_keys::{Address, FullViewingKey};
use fusion_num::Amount;
use fusion_view::{Storage, ViewServer};
use url::Url;

/// Knows things about a running fusion system, requires internet connectivity
#[derive(Clone)]
pub struct FusionKnower {
...
impl FusionKnower {
    /// Create the knower, loading or initializing the storage it uses for fusion data.
    ///
