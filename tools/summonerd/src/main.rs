...
use fusion_proof_params::{ProvingKeyExt, VerifyingKeyExt};
use fusion_proof_setup::all::combine;
use fusion_proof_setup::all::transition;
use fusion_proto::tools::summoning::v1::ceremony_coordinator_service_server::CeremonyCoordinatorServiceServer;
use fusion_proto::tools::summoning::v1::CeremonyCrs;
use fusion_proto::Message;
use std::fs;
use std::fs::File;
use std::io;
...
use crate::{fusion_knower::FusionKnower, server::CoordinatorService};
use fusion_proof_setup::all::{Phase1CeremonyCRS, Phase1RawCeremonyCRS};
...
Fusion summoning ceremony coordinator.
This tool orchestrates receipt of contributions for bootstrapping
proof parameters, to be used in the Fusion genesis event.
...
#[clap(
    name = "summonerd",
    about = "Fusion summoning ceremony coordinator",
    ...
    async fn exec(self) -> Result<()> {
        match self.cmd {
            ...
            Command::Start {
                ...
                let knower =
                    FusionKnower::load_or_initialize(storage_dir.join("fusion.db"), &fvk, node)
                        .await?;
